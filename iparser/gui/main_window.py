...
class MainWindow:
	...
	def __init__(self, root: tk.Tk):
		...
		# 剪贴板监控状态
		self.__monitoring_enabled = tk.BooleanVar(value=False)

		# 配置区域显示状态
		self.__config_visible = tk.BooleanVar(value=False)

		# 自定义配置文件路径
		self.__custom_config_file = Path('custom_config.json')

		# 初始化剪贴板监控器
		self.__clipboard_monitor = ClipboardMonitor(self.__root, self.on_clipboard_change)
		...
		self.toggle_monitoring() # 自动开启剪贴板监听

	# region UI related
	def create_ui(self):
		"""创建用户界面组件"""
		self.main_frame = ttk.Frame(self.__root, padding=(10, 0))
	...
	def create_control_area(self):
		...
		self.config_toggle_button = ttk.Button(
			control_frame,
			text='显示配置',
			command=self.toggle_config_area,
			width=10
		)
		self.config_toggle_button.pack(side=tk.RIGHT, padx=5)
	...
	def create_display_area(self):
		...
		self.original_entry = ttk.Entry(original_frame, width=20,
		...
		self.split_entry = ttk.Entry(split_frame, width=20, state='readonly',
	...
	def toggle_monitoring(self):
		"""切换剪贴板监听状态"""
		self.__clipboard_monitor.clear_clipboard()

		if not self.__monitoring_enabled.get():
			# 开启监听
			if self.__clipboard_monitor.start():
				self.monitor_button.config(text='停止监听')
				self.__monitoring_enabled.set(True)
		else:
			# 关闭监听
			if self.__clipboard_monitor.stop():
				self.monitor_button.config(text='开始监听')
				self.__monitoring_enabled.set(False)

	# region Config UI related
	def toggle_config_area(self):
		"""切换配置区域的显示与隐藏状态，并自动调整窗口尺寸"""
		if not self.__config_visible.get():
			# 显示配置区域
			self.config_frame.pack(fill=tk.BOTH, expand=True, pady=(0, 10))
			self.config_toggle_button.config(text='隐藏配置')
			self.__config_visible.set(True)
			self.resize_window()
		else:
			# 隐藏配置区域
			self.config_frame.pack_forget()
			self.config_toggle_button.config(text='显示配置')
			self.__config_visible.set(False)
			self.resize_window()

	def resize_window(self):
		"""根据内容自动调整窗口尺寸，确保平滑无闪烁"""
		# 获取当前窗口位置
		x, y = self.__root.winfo_x(), self.__root.winfo_y()
	...
	# endregion Config UI related

	# region Custom config file related
	def save_custom_config(self):
		...
		try:
			...
			with open(self.__custom_config_file, 'w', encoding='utf-8') as f:
				json.dump(config_data, f, ensure_ascii=False, indent=4)

			self.refresh_keywords()
			messagebox.showinfo('成功', f'配置已保存到：{self.__custom_config_file}')
			logger.info(f'配置已保存到：{self.__custom_config_file}')
	...
	def load_custom_config(self):
		"""从JSON文件加载自定义配置"""
		try:
			if self.__custom_config_file.exists():
				with open(self.__custom_config_file, 'r', encoding='utf-8') as f:
				...
				logger.info(f'已加载自定义配置：{self.__custom_config_file}')
		except Exception as e:
	...
	# endregion Custom config file related
	...
	def on_clipboard_change(self, content: str):
		...
		# 更新原始内容输入框
		self.original_entry.config(state='normal', style='TEntry')
		self.original_entry.delete(0, tk.END)
		self.original_entry.insert(0, content)
		self.original_entry.config(state='readonly', style='readonly.TEntry')
		...
		try:
			...
			# 更新分割结果输入框
			self.split_entry.config(state='normal', style='TEntry')
			self.split_entry.delete(0, tk.END)
			self.split_entry.insert(0, ', '.join(split_result))
			self.split_entry.config(state='readonly', style='readonly.TEntry')
			...
			# 复制解析结果到剪贴板
			self.__clipboard_monitor.set_clipboard(result)
			self.__clipboard_monitor.last_parsed = result

			logger.info(f'解析成功：{result}')
		except Exception as e:
			error_msg = f'解析失败：{str(e)}'
			self.result_entry.delete(0, tk.END)
			self.result_entry.insert(0, error_msg)

			# 复制原始内容到剪贴板
			self.__clipboard_monitor.set_clipboard(content)

			logger.error(error_msg)

	def on_close(self):
		"""窗口关闭时的处理"""
		self.__clipboard_monitor.stop()
		self.__root.destroy()
	# endregion Callback related
